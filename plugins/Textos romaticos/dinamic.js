...
const textosGolpear = [
  ...
  (from, who) => `*@${from}* 𝙂𝙊𝙇𝙋𝙀𝙊́ 𝘼 *@${who}* 𝙎𝙄𝙉 𝙈𝙄𝙎𝙀𝙍𝙄𝘾𝙊𝙍𝘿𝙄𝘼 👊\n━━━━━━━━━━━━━━━\n*@${who}*\n 𝙃𝙊𝙔 𝙉𝙊 𝙎𝙀 𝙇𝙀𝙑𝘼𝙉𝙏𝘼. 🤕`
]
...
async function handlerBailar(m, { conn, text }) {
  ...
  let imagenPath = './src/baile.jpg'
  
  await conn.sendMessage(m.chat, { react: { text: '💃🏻', key: m.key } })
  
  try {
    await fs.promises.access(imagenPath)
    const imageBuffer = fs.readFileSync(imagenPath)
    await conn.sendMessage(m.chat, { image: imageBuffer, caption: texto, mentions: [m.sender, userId] })
  } catch (e) {
    await conn.sendMessage(m.chat, { text: texto, mentions: [m.sender, userId] })
  }
}
...
async function handlerDarChocolate(m, { conn, text }) {
  ...
  let imagenPath = './src/chocolate.jpg'
...
async function handlerSaludar(m, { conn, text }) {
  ...
  let imagenPath = './src/saludar.jpg'
